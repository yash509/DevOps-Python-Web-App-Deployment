pipeline {
    ...
    stages {
        ...
        stage('Checkout from Git') {                        
            steps {                                       
                git branch: 'main', url: 'https://github.com/yash509/DevSecOps-Foodz-WebApp-Deployment.git'
            }
        }
        ...
        // stage('foodz-appll Dependencies') {
        //     steps {
        //         //dir('Band Website') {
        //             sh "npm foodz-appll"
        //         //}
        ...
        stage('OWASP File System SCAN') {
            steps {
                //dir('Band Website') {
                    dependencyCheck additionalArguments: '--scan ./ --disableYarnAudit --disableNodeAudit', odcfoodz-appllation: 'DP-Check'
        ...
        stage("Docker Image Tagging"){
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                            sh "docker tag foodz-app yash5090/foodz-app:latest " 
                        //}
                    }
                }
            }
        ...
        stage('Docker Image Scanning') { 
            steps { 
                sh "trivy image --format table -o trivy-image-report.html yash5090/foodz-app:latest" 
            } 
        } 
        ...
        stage("Image Push to DockerHub") {
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker') {
                            sh "docker push yash5090/foodz-app:latest "
                        //}
                    }
                }
            }
        ...
        stage('Docker Scout Image Scanning') {
            steps {
                script{
                   withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                       sh 'docker-scout quickview yash5090/foodz-app:latest'
                       sh 'docker-scout cves yash5090/foodz-app:latest'
                       sh 'docker-scout recommendations yash5090/foodz-app:latest'
                       sh 'docker-scout attestation yash5090/foodz-app:latest'
                   }
                }   
            }
        }
        ...
        stage("TRIVY"){
            steps{
                //dir('Band Website') {
                    sh "trivy image yash5090/foodz-app:latest > trivyimage.txt"   
                //}
            }
        }
        ...
        stage('Docker Scout Artifacts Analysis') {
            steps {
                script{
                  withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                      sh 'docker-scout sbom --format list yash5090/foodz-app:latest' // docker-scout sbom yash5090/foodz-app:latest
                    }
                }   
            }
        }
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop foodz-app | true"
                sh "docker rm foodz-app | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh 'docker run -d --name foodz-app -p 5000:5000 yash5090/foodz-app:latest' 
                //}
            }
        }
